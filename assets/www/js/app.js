...
// global variables
var db;
var shortName = 'GreRentalzDB1';
var version = '1.0';
var displayName = 'GreRentalzDB1';
...
function(transaction, result) {
  if (result !== null && result.rows !== null) {
    ...
    for (var i = 0; i < result.rows.length; i++) {
      ...
      $('#lbListProp #myTable').append('<tr><tbody><td>' + '<h3> Property Type </h3>'+ row.ppty_type + 
	  '</td>' + '<td>' + '<h3> Number of rooms </h3>'+ row.ppty_rooms + 
    ...
    db.transaction(function(transaction) {
        ...
        function(transaction, result){;
            ...
            for (var i = 0; i < result.rows.length; i++) {
              var row = result.rows.item(i);
                $('#pDetails').append(
				'<br />' + row.ppty_type + '<br />' + row.ppty_rooms + 
				'<br />' + row.ppty_furniture_type + 
				'<br /> £' + row.ppty_monthly_price  + 
				'<br />' + row.ppty_date + 
				'<br />' + row.ppty_notes + 
				'<br />' + row.ppty_VR +
				'<br />' + row.ppty_reporter);
                }
            }
        }, errorHandler);
}, errorHandler, nullHandler);
